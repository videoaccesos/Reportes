...
$result11 = "SELECT precio_mensualidad FROM privadas WHERE privada_id=$privada";
  $precioMensualidad= $conexion->query($result11); 

if ($precioMensualidad->num_rows > 0) //si la variable tiene al menos 1 fila entonces seguimos con el codigo
{
    while ($rowPrecio = $precioMensualidad->fetch_array(MYSQLI_ASSOC)) 
    {
$precioMes= $rowPrecio['precio_mensualidad'];
    }
}

$result22 = "SELECT folio FROM pago_mensualidades WHERE id_privada=$privada AND mes=$mes AND ano=$ano";
  $validarExiste= $conexion->query($result22); 

if($validarExiste->num_rows > 0){
echo "La privada no tiene mensualidades por agregar.";
}
else
{
$agregar= "INSERT INTO pago_mensualidades VALUES (null,$privada,$mes,$ano,$precioMes,0,$precioMes,'".$fecha."',2)";
  $finalMes1= $conexion->query($agregar);
}

$sql2="SELECT * from pago_mensualidades where id_privada = $privada and (estatus = 2 or estatus = 3) order by ano, mes  asc limit 1 ";
...
<form name="consulta_reporte" method="POST" action="imprimirContrato/vistas/folioMensualidad.php"  >
    <h3 style="text-align:center;color:#0040FF">Pago de Mensualidad</h3>
...
<a style="color:black;">Pago recibido:<a/>
...
<a style="color:black;">Tipo de Pago:<a/>
